    ...
    tokio::spawn(async move {
        ...
        if !wait_for_startup_remote_plugin_sync_prerequisites(codex_home.as_path()).await {
            warn!(
                ...
                "skipping legacy startup curated plugin sync because curated marketplace is not ready"
            );
            return;
        }
        ...
        match manager
            .sync_curated_plugins_from_legacy_remote(
                &config,
                auth.as_ref(),
                /*additive_only*/ true,
            )
            .await
        {
            Ok(sync_result) => {
                info!(
                    ...
                    "completed legacy startup curated plugin sync"
                );
                if let Err(err) =
                    write_startup_remote_plugin_sync_marker(codex_home.as_path()).await
                {
                    warn!(
                        ...
                        "failed to persist legacy startup curated plugin sync marker"
                    );
                }
            }
            Err(err) => {
                warn!(
                    error = %err,
                    "legacy startup curated plugin sync failed; will retry on next app-server start"
                );
            }
        }
    });
